...
public class NQueens {
    ...
    // 请不要超过32皇后问题，因为使用的是位运算，参数可以换成long类型，使用位运算做检查
    public static int num2(int n) {
        if (n < 1 || n > 32) {
            return 0;
        }
        // 如果是n皇后的问题就生成一个二进制的数。如果是8皇后问题，就申请一个后8位是1，前面都是0。如果是9皇后问题，就申请一个后9位是1，前面都是0。只使用位信息。
    ...
    // 将这三个位置求或，就知道这一行哪些位置不能填皇后了。使用位运算的特性，代替record的检查
    public static int process2(int limit,
        ...
        int res = 0;
        while (pos != 0) {
            mostRightOne = pos & (~pos + 1); // 把一个二进制的数中的最右侧的数提取出来
